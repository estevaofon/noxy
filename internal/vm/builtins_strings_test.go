...
func TestStringBuiltinsScalarTables(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "substring normal unicode runes", builtin: "strings_substring", args: []value.Value{value.NewString("aé🙂z"), value.NewInt(1), value.NewInt(2)}, want: value.NewString("é🙂")},
		{name: "substring empty", builtin: "strings_substring", args: []value.Value{value.NewString(""), value.NewInt(0), value.NewInt(1)}, want: value.NewString("")},
		{name: "substring short", builtin: "strings_substring", args: []value.Value{value.NewString("abc"), value.NewInt(1)}, want: value.NewString("")},
...
func TestStringsSplitBuiltin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			parts, ok := instance.Fields["parts"].Obj.(*value.ObjArray)
			if !ok {
				t.Fatalf("parts = %#v, want array", instance.Fields["parts"])
			}
