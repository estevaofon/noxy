...
Invocar `superpowers:requesting-code-review` (revisor com o diff `develop..HEAD` e o spec) e tratar os achados com `superpowers:receiving-code-review`.

> **Registro da execução (2026-08-20):** revisão de código (timebox 25 min) = "with fixes", 0 Critical. Important: (1) `OP_REF_INDEX` resolve `VAL_REF` também no caminho sem tag — mantido como defensivo e documentado no spec §6.2 (estado inalcançável por Noxy: `any` não guarda ref); (2) cobertura: adicionados `TestJSONLoadsMapRefValueSlots`, `TestRawValueInRefMapValueIsExplicitRuntimeError`, `TestEveryRefSlotHoldsRefOrNullAfterLegitimateWrites`, `TestImportedStructRefFieldForwardsThroughAnyBase` (`ref_slot_module_test.go`). Minor: `describeRefSlotIndex(idx, isMap)` → `for key 3` para map com chave int; demais (Load duplo no caminho de erro, helpers em `builtins_call_result.go`, follow-up de população in-place com `Owners > 1`) registrados, não alterados. **Armadilha do Step 4:** a primeira captura rodou durante uma edição de `executor.go` e ~12 exemplos gravaram o erro de compilação Go — refazer a captura só com a árvore estável; diferenças legítimas restantes são só não-determinismo (ordem de map, tempo, rand/uuid/crypto, endereços, cwd/binário, rede/dynamodb, waitgroup). Depois `superpowers:finishing-a-development-branch` — **opção "manter a branch, não abrir PR ainda"**: a PR #51 segue aberta no GitHub e o `develop` local não foi enviado; relatar ao usuário: commits da branch (`git log --oneline develop..HEAD`), resultado da suíte/runner/diff, e o texto de PR pronto (template Summary/Components/Test Plan do `~/.claude/CLAUDE.md`, título `fix/ref-slot-invariant - <descrição PT>`, base `develop`, label "not available to review") para ele abrir quando quiser.
