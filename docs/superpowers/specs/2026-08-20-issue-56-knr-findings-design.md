...
- `ensureFrameCapacity()` roda em `callPreparedClosure` **antes** de `&vm.frames[vm.frameCount]`: se `frameCount == len(frames)` e `len == FramesMax` → `runtimeError("stack overflow")` (texto inalterado); senão realoca, copia e **reaponta `vm.currentFrame = &vm.frames[frameCount-1]`**. Copiar `CallFrame` por valor preserva os headers de `Owned`/`Deferred` (capacidade reaproveitada — ver comentário atual em `vm.go:57-65`, que é atualizado).
- `push` cresce a pilha quando `stackTop == len(stack)`; no teto **panica com um sentinela tipado** (`errStackOverflow`) que `run()` (`executor.go:46`) recupera com `defer` e converte no erro de runtime padrão `stack overflow` (com posição do frame corrente). Só esse sentinela é recuperado; qualquer outro panic continua subindo como hoje. O `call_result` (`builtins_call_result.go:123`) já recupera panics na fronteira — o estouro dentro de `call_result` vira falha capturável, o que é coerente.
- **Realocação da pilha reaponta os upvalues abertos**: `vm.openUpvalues` é a lista de todos (e só) os `ObjUpvalue` abertos, e todos apontam para `vm.stack`; `growStack` percorre a lista e chama um novo `(*ObjUpvalue).Relocate(old, new []Value)` (pacote `value`) que recalcula `location = &new[idx]` com `idx` derivado do ponteiro antigo. `REF_UPVALUE` refs guardam o `*ObjUpvalue`, não o ponteiro cru — nada a fazer. `REF_PTR` não tem site de criação (só leituras em `executor.go:224`, `references.go:92`, `stack.go:108`) — permanece morto e inalterado.
- **Auditoria obrigatória de `*CallFrame` e de fatias de `vm.stack` mantidos em variáveis Go através de uma chamada que pode reentrar na VM** (crescimento invalida o ponteiro/fatia antiga):
  ...
  - `unwind.go:22-34`: o laço que executa `frame.Deferred` segura `frame := vm.currentFrame` enquanto roda defers (que reentram a VM) → passa a **reobter o frame por índice** a cada iteração.
  - `calls.go:143` (`frame := &vm.frames[...]` depois de `ensureFrameCapacity` — ok), `task_execution.go:84`, `references.go:176,217`, `runtime_errors.go:119` (laços sem chamada — ok).
  - `callNative` passa `args := vm.stack[a:b]` a natives que podem reentrar (comparadores, `call_result`): leitura da fatia antiga continua correta (os valores são cópias por valor e ninguém escreve nela) — documentar no código; não copiar.
- Documentação: spec §13 ("profundidade de chamada limitada só por memória: 100 000 frames / 1 048 576 slots → erro `stack overflow`"), AGENTS.md §Segurança (trocar `vm.sp >= StackMax` pela descrição nova).

**Testes.** `depth(10000)` (issue) e `depth(50_000)` devolvem o valor; recursão infinita → erro de runtime `stack overflow` (não panic, exit 1, sem trace Go); lista encadeada recursiva (`noxy_examples/linked_list.nx` padrão `push_back`) com 1 000 nós; closure que captura um local **antes** de uma recursão profunda (força crescimento da pilha) e lê/escreve o upvalue depois (valor correto — prova do `Relocate`); `ref` a local (REF_UPVALUE) através do crescimento; defer registrado em frame externo que roda após recursão profunda (prova do reobter em `unwind`); VM de task nasce com `len(stack) == stackInitial`; adaptar `defer_test.go:304-323` e `builtins_tasks_test.go:481` (indexam `StackMax-…` direto) para `len(machine.stack)`. **Benchmark**: `BenchmarkNoxyCallOverhead` (`call_alloc_bench_test.go`) antes/depois, rodadas intercaladas — custo por chamada não pode regredir além do ruído (o teste de capacidade é uma comparação por chamada/push).
...
- `SharedState` ganha `stdin *bufio.Reader` criado uma vez (`sync.Once`) sobre `os.Stdin`; `input()` usa esse leitor. Semântica de `input()`: devolve a linha sem `\r\n`/`\n`; no EOF **com** texto parcial devolve o parcial; no EOF **sem** texto devolve `""` (como hoje — a distinção fica com `io.stdin()`).
...
- `FileResource` ganha `reader *bufio.Reader` criado sob demanda (usado por `read_line`, §4). Para o recurso de stdin, `io.read`/`io.read_lines` leem **o restante** via `io.ReadAll(reader)` (não há `Stat().Size()` útil em pipe); em arquivos comuns continuam lendo do início como hoje. Documentar em §12 da spec: "`read`/`read_lines` leem o arquivo inteiro desde o início; `read_line` é incremental a partir da posição corrente — não misture os dois no mesmo handle de arquivo".
...
- `io_list_dir(path, IOLinesResult)`: `os.ReadDir` → `data` = nomes (sem caminho, ordem de `ReadDir` = ordenada por nome), `ok=false, error=<os error>` em falha. **`io.nx` muda a assinatura** de `list_dir(path) -> IOResult` (quebrada, `data: string` nunca funcionou) para `-> IOLinesResult`.
...
- **Migração dos exemplos**: rodar o runner; todo `.nx` que compilar com esse erro é corrigido com a comparação explícita (listar no CHANGELOG).
...
- **8c (tipo nominal único)**: `areTypesCompatible` (`compiler.go:2849`) — depois da comparação por `String()`, dois `*ast.PrimitiveType` são compatíveis se nomeiam a **mesma declaração de struct**: o compilador passa a registrar a **origem** de cada struct importado (`c.structOrigins[name] = module`, preenchido em `importModuleStructs`); um nome qualificado `ns.Name` resolve via `c.namespaceImports[ns]` → módulo; dois nomes são o mesmo struct se o nome base é igual **e** o módulo de origem é igual (struct local, sem origem, só é igual a si mesmo). Mensagens de erro inalteradas. Em runtime o `ObjStruct` já é o mesmo objeto (módulo carregado uma vez), então nada muda na validação de tipo de runtime.
...
- Compilador: `Loop` ganha `ContinueJumps []int`. `case *ast.ContinueStmt`: `continue outside of loop` se `len(c.loops)==0`; emite os `OP_POP` dos locais além de `EnclosingLocals` (igual ao `break`); no **`while`** o alvo é `loopStart` (reavaliar a condição) → `emitLoop(loopStart)` direto; no **`for`** o alvo é o início do passo "10. Increment Index" (`compiler.go:~1552`, logo depois do `endScope()` do corpo — a variável do laço já foi estourada pelo `continue`) → `emitJump(OP_JUMP)` registrado em `ContinueJumps` e patchado ali. Laços aninhados: `c.loops` topo.
...
**Testes.** Compilador: `continue` em `while`, em `for` (array, map, string), aninhado, dentro de `if` com `let` no corpo (pops corretos — pilha balanceada), fora de laço (erro). VM e2e: saídas esperadas (ímpares de 1..9, etc.); `loop_break_test.go` dos dois pacotes serve de modelo.
...
- `*` unário em tipo estático **não-ref** (`PrefixExpression`, `compiler.go:1292-1323`) → `[line N] cannot dereference non-reference value of type int` (`2 ** 3` passa a ser erro; tipo desconhecido continua passando para o `OP_DEREF` de runtime).
...
- Format spec em f-string (14); `.5` e semântica octal `031` (11); `io.stdout()`/`io.stderr()` como `File` (6 — `eprint` cobre); overflow checado (16); erro de parser para `}` isolado em f-string.
- Mudar `io.read`/`read_lines` de "ler do início" para "ler da posição corrente" em arquivos comuns (só stdin lê o restante — §3).
...
- [ ] Item 1: `depth(10000)`/`depth(50000)`; overflow infinito → erro de runtime; upvalue/ref/defer através do crescimento; tasks nascem pequenas; benchmark sem regressão
...
- [ ] Item 8: erro `m.x = v`; `geometry.Point` ≡ `Point`; negativo struct local
- [ ] Item 9: `continue` em while/for/aninhado/fora de laço; walkers de generics
...
- [ ] Item 14: `{{`/`}}`; erro para `:spec` com hint
