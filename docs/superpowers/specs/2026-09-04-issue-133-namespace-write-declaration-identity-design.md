...
`ast.PrimitiveType` ganha um campo:
...
O módulo declarante **não** vai no nó: é derivado por `c.structOrigin(decl)` (`moduleDiscovery.origins`, já por ponteiro; `""` = programa), a única fonte de verdade — um campo `Origin` gravado pelo validador do módulo diria `""` e divergiria da visão do importador (revisão independente, achado 8). `String()` continua devolvendo `Name`. `walk.go` não muda (tipos não são visitados). `ast.CloneType` e `substituteType` (`generics_substitute.go`) **copiam o ponteiro** `Decl` — clonar a declaração quebraria a identidade em silêncio.
...
- `areTypesCompatible` / `areStrictTypesCompatible` e a checagem de dependência de template importado (`generics.go` ~886, `definedType.String() == importerType.String() || typesEquivalent`): o atalho por string **sai** dos três — dois `Decl` distintos podem exibir a mesma string (alias `use other as base` + `base.V` canônico de um módulo `base` não importado). O primeiro teste passa a ser `typesEquivalent`; o custo é desprezível (o atalho só evitava a recursão estrutural). `compiler.go` ~3630 (`t.String() == name` contra lista de nomes de primitivos) não envolve struct e fica.
...
`resolveAnnotation` (`generics_structs.go`) — já chamado em toda posição de anotação do AST: `let` (`compiler.go` ~412), parâmetros e retorno (`resolveSignatureAnnotations`, inclusive literal de função e instanciação de template), campos de struct (`resolveStructFieldAnnotations`); não há outras posições (`for`, `select`/`case` e `catch` não anotam tipo). Ganha, **antes** do teste `needsAnnotationResolution`, um passe `bindStructDecls(t)` que percorre a anotação **in place, sem alocar nó algum**: para cada `*ast.PrimitiveType` com nome que não é builtin, não é nome de instância genérica (`isGenericInstanceName`, ver 1.6) e tem `Decl == nil`, resolve por `structDeclaration(Name)` (nome simples via `c.structs`, `ns.T` via namespace) e grava `Decl`. Os nós compostos (`P[]`, `ref P`, `P?`, `map`, `chan`, `func`) são só atravessados. O fast path de custo zero (§5 dos genéricos) e o contrato "mesmo ponteiro de volta" ficam intactos: `needsAnnotationResolution` **não muda** (continua `false` para todo `PrimitiveType`), então nenhuma anotação sem genéricos passa a alocar (revisão independente, achado 1). O passe é idempotente — `resolveStructFieldAnnotations` roda duas vezes de propósito (`function_types.go` ~424) e a segunda passagem não encontra `Decl == nil` em nada que a primeira já tenha resolvido.

Nome que não resolve fica com `Decl == nil` e é reportado por `checkDeclaredType` como hoje (`unknown type 'V'`, `cannot resolve type 'base.V'`), com o hint de 1.7. Depois da resolução, `Decl == nil` só existe em primitivo, em instância genérica (1.6) e em anotação inválida já reportada.

Efeito colateral desejado: a mutação in place acontece no AST memoizado do módulo durante o `validator.Compile` de `loadModuleDeclarations` — a mesma disciplina que `resolveStructFieldAnnotations` já pratica. O `Decl` gravado ali aponta para o mesmo `*ast.StructStatement` que `discoverModuleStructs` devolve ao importador (mesmo AST), então a identidade atravessa unidades de compilação, e `ModuleState` já carrega o `moduleDiscovery` entre linhas do REPL. O VM recompila cada módulo com um compilador próprio sobre o próprio AST do módulo (`modules.go` ~205), onde `Decl` aponta para as declarações desse mesmo AST — autoconsistente. `programViewType` (1.5) **recalcula** `Name` a partir de `Decl` na visão do importador; `Decl` é a única coisa carregada.

### 1.5 `programViewType` e `programStructName` viram só exibição

`programViewType(t, origin)` deixa de devolver `(nil, false)` para struct. A ordem dos testes no caso `*ast.PrimitiveType` é: builtin → `isGenericInstanceName` (devolve `(nil, false)`, 1.6) → struct por `Decl` ou, se ausente, por `lookupStructFrom(origin, Name)`. Para struct devolve **sempre** um nó novo `{Name: programStructName(decl), Decl: decl}`. `programStructName(decl)` passa a devolver sempre uma grafia, nesta ordem:
...
2. `alias.Nome` para o primeiro `use m [as alias]` de `namespaceOrder` cujo módulo exporta a declaração **e cujo alias não está sombreado por local ou upvalue no ponto em que o tipo é produzido** (`isShadowedByLocal`) — é o caso (c) da issue: dentro de `func f(m: Box)`, `let p = w.make()` tipa `p` como `w.V`, não `m.V`, e a mensagem sobre `p` exibe `w.V`;
...
**A grafia é fixada quando o tipo é produzido, não quando é impresso.** Um `let v = w.make()` de topo grava `w.V` em `c.globals`; uma mensagem sobre `v` dentro de `func f(w: Box)` imprime `w.V` mesmo com `w` sombreado ali — não há re-tradução na impressão (revisão independente, achado 3). É o mesmo compromisso do #126 e a identidade não é afetada. Do mesmo modo, a tradução do caminho `select` (abaixo) roda no predeclare, em **ordem textual** dos statements: só os `use` **anteriores no texto** contam para a regra 2 — `use mid select mkv` seguido de `use base as b` exibe `base.V` (canônico), a ordem inversa exibe `b.V` (achado 4). A spec §11 documenta "the first alias declared before that point".
...
`newStructFunctionType` (construtor) passa a receber a declaração e produzir retorno `{Name: decl.Name, Decl: decl}` — os quatro chamadores (declaração local, predeclare, instância genérica, `importBindingFrom`/`importedBindingType`) têm a declaração à mão.
...
**Nó órfão com grafia canônica.** Todo site que reconstrói um `PrimitiveType` a partir de `.Name`/`.String()` de outro tipo (não só `CloneType`/`substituteType`) tem de carregar `Decl` junto: um nó `{Name: "base.V"}` sem `Decl` é pior que hoje, porque `structDeclaration("base.V")` não resolve (`base` não está em `namespaceImports`) e `typesEquivalent` cai no fallback por nome e falha (achado 6). O §5 lista o grep e o teste de caracterização.
...
- Anotação que não resolve continua erro. `unknown type 'V'` ganha hint que nomeia o módulo real quando alguma dependência já carregada declara `V`, em dois passos: (1) a **origem** vem de `moduleDiscovery.origins` (mapa `decl → módulo declarante`, por ponteiro), varrido por `decl.Name == "V"`; (2) os **reexportadores** vêm de `namespaceOrder` × `discoverModuleStructs(módulo)["V"] == decl` (`origins` aponta sempre para o declarante, nunca para quem reexporta — achado 7). Com origem `base` e reexportador `mid` importado por namespace, o hint é `add 'use base' or 'use mid select V' to name this type`; só origem: `add 'use base' or 'use base select V'`; sem candidato, o hint atual (`declare 'struct V' or import it with 'use m select V'`). `cannot resolve type 'base.V'` fica como está.
...
- `setPropertyGeneric` (`field_ops.go`): base `*value.ObjMap` → `old, exists := m.Get(name)`; ausente é `undefined property '%s' in module/map` (mesma mensagem de `getPropMutGeneric`); `value.Retain(new)`, `m.Set(name, new)`, `value.Release(old)` — retain-antes-de-release, como o ramo de instância. **`ObjMap.Set` não toca RC** (`map.go`: só `store.set`), então o ramo conta uma vez, não em dobro (achado 9). O store compartilhado é mutexado e avança `gen`, então a garantia "operações individuais em global são sincronizadas" de `docs/concurrency.md` se mantém e o cache de leitura por geração invalida.
- `REF_PROPERTY` com contêiner `*value.ObjMap` (`references.go`): `descend` (passo intermediário, `forWrite` uniciza o filho e regrava com Retain/Set/Release, como o ramo de mapa de `REF_INDEX`) e `resolveReference` (passo final, devolve `referenceSetter{kind: setterMap, mapping, key: name}`) ganham o ramo. O RC da escrita final **não** fica no setter: `referenceSetter.set` para `setterMap` só faz `mapping.Set`; retain-antes-de-release é o funil único de `storeReferenceValue` (`references.go` ~517-550), que já atende `setterMap`. `borrowBaseAddr` já cobre. A guarda R1 de "slot já guarda referência" para `ObjMap` consulta o valor guardado (`VAL_REF`), como `REF_INDEX` faz.
...
- `struct_identity_test.go` (novo): `Decl` preenchido por `resolveAnnotation` em `let`/parâmetro/retorno/campo, inclusive dentro de `P[]`, `ref P`, `P?`, `map`, `func` sem alocar nó novo (mesmo ponteiro de volta); instância genérica (`main::Caixa<int>`) **não** recebe `Decl`; `typesEquivalent` por ponteiro ignorando `Name`; dois `Decl` distintos com a mesma grafia não são compatíveis (inclusive na checagem de template importado de `generics.go` ~886); `CloneType`/`substituteType` preservam o ponteiro; struct local `V` ≠ `base.V`; **caracterização**: um nó `{Name: "base.V"}` sem `Decl` não equivale à declaração (documenta por que todo site reconstrutor tem de carregar `Decl`).
...
- `namespace_member_typing_test.go`: `TestNamespaceMemberStaysDynamicWhenStructIsUnnameable` e `TestNamespaceMemberReexportedByWildcardStaysDynamic` invertem; novos: `mid.mkv("x")` e `mid.mkv(1, 2)` são erros, `let s: string = v` é erro, `v.x` checado, `mid.usa(v)` confere; `let p = w.make()` **dentro** de `func f(m: Box)` tipa e exibe `w.V` (a inferência é no escopo que sombreia); `let v = w.make()` de topo continua exibindo `m.V` dentro de `f` (grafia fixada na produção — caracterização); `use mid select mkv` antes de `use base as b` exibe `base.V`, depois exibe `b.V` (ordem textual — caracterização); `let q: m.V` dentro de `f(m: Box)` compila.
...
- lista de suspeitos, cada um revisado (é struct? então passa a receber a declaração): `grep -n 'PrimitiveType{Name:' internal/compiler` (23 sites em `compiler.go`, 4 em `function_types.go`, 3 em `call_result.go`, 1 em `member_types.go`, mais `builtin_calls.go`/`builtin_return_types.go`), `grep -n 'Name: [a-z]*\.\(Name\|String()\)' internal/compiler` (reconstrução a partir de outro tipo — o nó órfão de 1.5) e `grep -n 'String() ==' internal/compiler` (atalho por string remanescente; `containsCallableType` com `visiting` por nome entra aqui);
