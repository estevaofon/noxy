...
| `parseStructStatement` nome de campo | `curToken.Type != IDENTIFIER` → pula em silêncio | `isNameToken(curToken.Type)`; senão **erro** `expected identifier, found X` e retorno `nil` |
...
Com `use src.map as map` legal, `let t: map.Tile = map.mk()` precisa parsear. Em `parseAtomicType`, um token de tipo contextual seguido de `DOT` é lido como nome qualificado cujo namespace é o literal da keyword, reutilizando o ramo de `IDENTIFIER` (`name += "." + ...`, genéricos `Nome<...>` inclusos). `map[` e `chan T` seguem inalterados: o lookahead de um token decide. `int.X` e afins também caem aqui — em posição de tipo um primitivo nunca é seguido de `.`, então não há ambiguidade.
...
Novo tipo em `internal/token/token.go` (grupo "Especiais"), `Display()` = `lexer error`. Todo ramo de `NextToken` que hoje grava uma razão de `readQuoted` em `ILLEGAL` (string, string de aspas simples, bytes, f-string) grava em `LEXER_ERROR`. `ILLEGAL` fica **só** para caractere desconhecido, produzido pelo `else` de `NextToken`. `lexer.IsReason` é removida; `noPrefixParseFnError` decide por tipo:
