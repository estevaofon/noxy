...
- **CHANGELOG** em `## [Unreleased]` (o release data a seção, como no #133): `Added` — keywords de tipo contextuais; identificadores Unicode. `Changed` — caractere ilegal diagnosticado por runa (um `invalid syntax` por caractere); coluna dos diagnósticos em caracteres, não bytes; `lexer.IsReason` sai (API interna). ~~Sem `BREAKING`: só programas que não compilavam passam a compilar, e a coluna muda só onde já estava errada.~~ **Emenda (revisão final, 2026-09-04):** a afirmação estava errada para o campo de struct. `struct P` com campo `ref: int` ou com uma linha `5: int` **compilava** antes (o token era descartado em silêncio) e agora é `SyntaxError: expected identifier, found ref`; é quebra e vai em `Changed (BREAKING)` com tabela Antes/Agora e migração (renomear o campo ou remover a linha), como o AGENTS.md exige. O erro de campo inválido é **um** por struct: o parser consome até o `end` (`skipUntilEnd`, o mesmo padrão dos parâmetros de função), não uma cascata. A nota de §6 não fixa número de versão ("in earlier versions"): o bump é decisão do release.
