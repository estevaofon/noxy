...
**Status:** aprovado em conversa, spec para revisão · **Issue:** a abrir · **Relação:** spec de extensões por processo (`2026-08-29-process-extensions-design.md`, §8 e §15: "versioned `noxy.sum` keys" e "`noxy.sum` format spec" eram pendências declaradas); spec wasm (`2026-08-23`, §15 idem).
...
| `noxy.mod` do repositório | `require github.com/estevaofon/quicksort v0.1.0` e `noxy_dynamodb v0.3.0`; o `noxy.mod` do quicksort **requer a si mesmo** em `HEAD` |
...
| Resolução de `use` (`internal/vm/modules.go`) | `NOXY_PATH`, depois `<RootPath>/noxy_libs/<a/b/c>[.nx]`, `stdlib`, raiz, embutido; falha com `module not found: <nome>` sem menção ao `noxy.mod` |
| CI (`.github/workflows/network-deadlines.yml`) | testes Go e o runner `noxy_examples/run_all_tests_concurrent.nx` em ubuntu e windows; `use_quicksort.nx` e `test_libs.nx` **rodam** no runner (não estão em `exclusions`); os exemplos de dynamodb e space invaders estão excluídos |
...
|---|---|---|
| `noxy.mod` é intenção (só diretas); `noxy.sum` é o lock (fechamento inteiro) | `pyproject.toml`/`uv.lock`; `Cargo.toml`/`Cargo.lock` | `// indirect` no `noxy.mod` (Go ≥ 1.17): polui o manifesto que o usuário edita à mão |
...
| Conflito de versões resolve pela maior (MVS) | Go MVS | primeira visitada vence (hoje): depende da ordem do mapa |
...
| `--sync` só apaga o que ele mesmo instalou (carimbo) | uv registra o que gerencia no `.venv` | apagar tudo que não está no lock: destruiria `math_lib` |
| `--locked` recusa mudar o lock | `cargo --locked`, `uv sync --frozen`, `GOFLAGS=-mod=readonly` | — |
...
- `noxy <versão>`: `--sync` compara com `version.Version`; binário **mais antigo** que o exigido é erro `noxy.mod requires noxy v0.24.0; this is v0.23.5` (precedente: diretiva `go` desde 1.21). `--sync` nunca escreve essa linha; `--get` segue sobrescrevendo com a versão corrente, como hoje.
...
- `require` de um pacote a si próprio (`module quicksort` + `require .../quicksort`) é ignorado na resolução, sem aviso: o `quicksort` publicado já faz isso.
...
- **Chave é o caminho do módulo** do `noxy.mod` (`github.com/...`), não o caminho local. A conversão local ↔ módulo fica num único par de funções em `pkgmanager` (`LocalPath(module)`, já existe como `localPackagePath`; `ModulePath(local)` nova: troca `_` por `.` no **primeiro** segmento — hostname não tem `_`), usadas pelo `--get`/`--sync` e pela VM.
- **Invariante: uma versão por módulo.** MVS produz exatamente uma; `Save` recusa gravar duas. A VM, que só conhece o diretório em disco, procura o artefato **por módulo e arquivo, ignorando a versão**; o invariante torna a busca unívoca.
...
Uma linha por pacote que o `--sync`/`--get` instalou: `<módulo> <versão>`. Só serve para a poda (§5.3): diretório presente no carimbo e ausente do lock novo é removido; diretório fora do carimbo (`math_lib`) nunca é tocado. Carimbo ausente ou corrompido é tratado como vazio (nada é podado, aviso em stderr). O arquivo é reescrito inteiro ao final de cada sync.
...
| Pedido | Resolve para |
|---|---|
| `pkg@v1.2.3` | a própria tag (com ou sem `v`; gravada como veio) |
| `pkg` ou `pkg@HEAD` | tag semver mais nova (`git ls-remote --tags`, `newestSemverTag` já existente); sem tag semver → pseudo-versão do commit apontado por `HEAD` |
| `pkg@<sha>` ou `pkg@<branch>` | pseudo-versão do commit resolvido |

Pseudo-versão: `v0.0.0-<yyyymmddhhmmss UTC do commit>-<12 hex do sha>`, obtida com `git log -1 --format=%ct %H` no clone temporário. Clone de pseudo-versão faz `git checkout <sha12>`. Extensão por processo continua exigindo tag (assets pendem de release): pseudo-versão para `kind = "process"` é erro, mensagem de hoje.

Ordem entre versões é a do semver: pseudo-versão (`v0.0.0-…`) é pré-release e fica abaixo de qualquer tag; duas pseudo-versões comparam pelo timestamp. Isso substitui o "tag contra pseudo-versão é erro" da conversa: a ordem semver já decide, sem caso especial.
...
Um `--sync` num projeto já sincronizado não toca a rede e não reescreve arquivo cujo conteúdo não mudou.
...
| `sync.go` (novo) | §5.1–5.3: `Sync(root string, opts SyncOptions) error`, carimbo, poda, saída |
...
| `internal/vm/modules.go`, `extensions.go` | §6 |

Funções de git e rede continuam atrás das costuras existentes (`gitURLFor`, `resolveNewestTag`, `httpClient`); uma nova, `gitCommitInfo`, para a pseudo-versão.
...
- **`resolve_test.go`**: repositórios git locais (helper `gitIn` existente) formando um grafo `raiz → a@v1, b@v1; b → a@v2` → escolhe `a@v2` e imprime o aviso; auto-`require` ignorado; `HEAD` sem tag vira pseudo-versão e é gravada no `noxy.mod`; pacote sem `noxy.mod` é folha.
...
- **VM**: `modules_test` para a dica `required by noxy.mod`; `process_extensions_e2e_test` com a string nova; teste de `verifyExtensionSum` com chave por módulo.
...
**Added**: `--sync`, `--locked`, hash de árvore, MVS, pseudo-versão, carimbo, dica no `module not found`.
